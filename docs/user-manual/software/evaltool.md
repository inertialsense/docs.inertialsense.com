...
1. Select the update type using the drop down menu:
   ...
   | -------------- | ------------------------------------------------------------ |
   | FwPkg (.fpkg)  | Batch firmware update method for updating multiple devices in one process.  The .fpkg file contains multiple firmware files and instructions for sequencing firmware updates for all available devices. |
   | GPX-1 (.bin)   | GPX-1 firmware update.                                       |
   | IMX-5.0 (.hex) | IMX-5.0 firmware update that used the legacy InertialSense bootloader (ISB). |
