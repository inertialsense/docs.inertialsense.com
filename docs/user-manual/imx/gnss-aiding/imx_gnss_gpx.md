...
The IMX can be configured for use with the Inertial Sense GPX-1 multi-band GNSS receiver.  This can be done using either the EvalTool GPS Setting tab or the IMX `DID_FLASH_CONFIG.ioConfig` and `DID_FLASH_CONFIG.RTKCfgBits` fields.    

| GPS Ports      | Value                                       |
| -------------- | ------------------------------------------- |
| GPS Source     | serial 0, serial 1, or serial 2             |
| GPS Type       | GPX-1                                       |
| GPS1 Timepulse | *Disable* or IMX pin connected to GPX-1 PPS |

| RTK Rover    | Value                       |
| ------------ | --------------------------- |
| GPS RTK Mode | Precision Position or Compass |
...
The following sections detail how to interface and configure the IMX for operation using the GPX-1.  See [RTK precision positioning](../../gnss/rtk_positioning_overview.md) and [RTK compassing](../../gnss/rtk_compassing.md) for RTK operation principles.  
