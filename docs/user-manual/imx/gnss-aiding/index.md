...
If the setup includes a significant distance (40cm or more) between the GNSS antenna and the IMX central unit, enter a non-zero value for the GNSS antenna lever arm, `DID_FLASH_CONFIG.gnss1AntOffset` (or  `DID_FLASH_CONFIG.gpsAnt2Offset`) X,Y,Z offset in meters from [Sensor Frame](../application-config/imu_ins_configuration.md#coordinate-frame-relationship) origin to GPS antenna.  The Sensor Frame origin and Hardware Frame origin are always at the same location but may differ in direction according to the Sensor Rotation.
